    ...
    <div className="w-full h-full bg-black bg-opacity-40 absolute top-0 left-0 flex justify-center items-center z-50">
      <div className="bg-white p-6 rounded-md w-96 max-h-[80vh] shadow-lg">
        <div className="flex justify-between items-center mb-4">
          ...
          <button onClick={onClose} className="text-2xl text-gray-500 hover:text-gray-800 focus:outline-none">
            &times;
          </button>
        </div>
        {menus && menus.length > 0 ? (
          <div>
            {menus.map((menu, index) => (
              <div key={index} className="py-2 flex justify-between" onClick={() => handleMenuTap(menu)}>
